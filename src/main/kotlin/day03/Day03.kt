...
fun String.rankDoubles(): Int {
    val a = this.substring(0, this.length / 2).toSet()
    val b = this.substring(this.length / 2).toSet()
    val doubles = a.intersect(b)
    return doubles.sumOf { it.rank() }
}

fun Triple<String, String, String>.rank(): Int {
    val (a, b, c) = this
    val group = a.toSet().intersect(b.toSet()).intersect(c.toSet())
    return group.sumOf { item -> item.rank() }
}

fun <T> List<T>.toTriples() = this.zipWithNext().zipWithNext { a, b -> Triple(a.first, a.second, b.second) }
    .filterIndexed { index, _ -> index % 3 == 0 }

fun Char.rank(): Int = if (this in 'a'..'z') {
    this - 'a' + 1
} else {
    this - 'A' + 27
}

private const val input = """dtddvvhwttHJhwdhJPddhwJGppmGjgpQgTjQplQpTljwpg
